...
def buy_call_option(symbol, portfolio_value, max_contracts=1):
    """
    ...
    try:
        ...
        # Apply max_contracts guardrail from scheduler
        capped_contracts = min(contract["num_contracts"], max_contracts)
        if capped_contracts < contract["num_contracts"]:
            print(f"📈 OPTIONS: Capping contracts {contract['num_contracts']} → {capped_contracts} (guardrail)")

        order_body = {
            "symbol": contract["contract_symbol"],
            "qty": str(capped_contracts),
            "side": "buy",
            "type": "limit",
